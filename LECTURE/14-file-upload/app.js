...
const uploadDetail = multer({
    storage: multer.diskStorage({
        ...
        filename(req, file, done) {
            ...
            done(null, path.basename(originalname, ext) + Date.now() + ext); // 저장할 파일명
        }
    }),
...
app.post("/upload", uploadDetail.single('userfile'), (req, res) => {
    ...
    // {
    //     fieldname: 'userfile',
    //     originalname: '6ì\x9B\x94 ì\x9B\x94í\x8E\x98ì\x9D´í\x8D¼.png', // multer 최신 버전의 버그. 한글이 안 읽힘.
    //     encoding: '7bit',
    //     mimetype: 'image/png', // 확장자
    //     destination: 'uploads/', // 파ㄹ이 저장된 폴더 
