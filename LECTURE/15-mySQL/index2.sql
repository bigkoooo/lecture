...
-- fk 설정을 할 건데, customer 테이블에 custid가 원본이야.
-- 열 이름(custid)은 달라도 되는데(pk/fk), 맞춰주는 게 좋음. 
...
select * from customer where custname like '%수'; -- x => 예외. 이 수, 남궁민수 다 해당됨.
...
select * from customer where phone not like '%_4'; -- %4?
--  %4: 4, 14, 204, 1004 ...
-- %_4: 14, 204, 1004 .... 끝자리가 무조건 4인 경우만!
...
select count(distinct custid) from orders; -- 가입한 회원 중 구매전환이 이루어진 고객 수

-- 9. group by: "~별로"
-- 고객별로 주문한 주문 건수 구하기
-- select 절에서는, group by 에서 사용한 속성 과 집계함수만 사용 가능
